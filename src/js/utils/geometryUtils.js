...
/**
 * Calculate bounding box of a set of coordinates (assumed to be a single ring).
 * @param {Array<[number, number]>} coords
 * @returns {{ minLon: number, minLat: number, maxLon: number, maxLat: number }}
 */
export function getBbox(coords) {
  // Flatten coordinates regardless of polygon or multipolygon structure
  ...
  const process = (array) => {
    for (const element of array) {
      if (Array.isArray(element[0])) {
        // element is a nested array (e.g., ring or polygon), recurse
        process(element);
      } else {
        // element is a coordinate pair [lon, lat]
